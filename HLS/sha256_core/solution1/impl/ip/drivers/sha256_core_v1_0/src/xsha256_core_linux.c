...
typedef struct {
    ...
    XSha256_core_uio_map maps[ MAX_UIO_MAPS ];
} XSha256_core_uio_info;

/***************** Variable Definitions **************************************/
static XSha256_core_uio_info uio_info;
...
static int uio_info_read_name(XSha256_core_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSha256_core_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSha256_core_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSha256_core_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSha256_core_Initialize(XSha256_core *InstancePtr, const char* InstanceName) {
	XSha256_core_uio_info *InfoPtr = &uio_info;
